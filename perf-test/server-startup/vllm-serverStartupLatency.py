...
class Collector:
    def __init__(self, kubeconfig=None, k8s_context=None):
        ...
        self.core_v1_api, self.app_v1_api, self.custom_objects_api = (
            self.load_kubeconfig(kubeconfig, k8s_context)
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--config",
        default="~/.kube/config",
        help="kubeconfig for the target cluster",
    )
    parser.add_argument(
        "-ctx", "--context", default=None, help="name of k8s cluster context"
    )
    parser.add_argument(
        "-c", "--containername", default=None, help="name of container in VLLM pod"
    )
    parser.add_argument(
        "-l", "--label", help="vllm pod label selector (e.g, 'environment=test')"
    )
    parser.add_argument(
        "-n", "--namespace", default="default", help="vllm pod namespace"
    )
    parser.add_argument(
        "-m",
        "--model",
        default="mymodel",
        help="helpful description of model run by VLLM",
    )
    parser.add_argument(
        "--model-cached",
        default="no",
        ...
        for pod in pods:
            ...
            if pod.status.phase == "Running" and any(
                ...
                if pod_logs:
                    ...
                    t5 = round(
                        float(tx) - float(t6), 3
                    )  # computing time before torch.compile
